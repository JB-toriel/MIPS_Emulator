...
# Usage string
USAGE="Usage: `basename $0` -e <executablefile> [-b] [-p] [-d] [-q] [-v] [-z] [-o <summaryfile>]  [-c] [-s <skip_file>] <args_testfiles>"
...
printHelp()
{
	...
	echo "	-z				: pause mode : adds some delay in between two summary"
...
parseCommandLine() {
    #option
	while getopts e:o:hbqcdxwvpsz: OPT
	do
		case "$OPT" in
		h)
			printHelp
...
executeTest()
{
	...
	TEST_RETURN_CODE=$(sed -ne '0,/^#.*TEST_RETURN_CODE\s*=\s*\([A-Z]*\).*/s/^#.*TEST_RETURN_CODE\s*=\s*\([A-Z]*\).*/\1/p' $test_file_name)

	TEST_COMMENT=$(sed -ne '0,/^#.*TEST_COMMENT\s*=\s*\(.*\).*/s/^#.*TEST_COMMENT\s*=\s*\(.*\).*/\1/p' $test_file_name)
